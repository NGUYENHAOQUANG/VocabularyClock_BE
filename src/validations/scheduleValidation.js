...
export const createScheduledTaskSchema = z.object({
  ...
  time: z.string({ required_error: "Time is required" }).regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "Invalid time format (HH:MM)"),
  methods: z.array(z.enum(['flashcard', 'meaning', 'write_full_word', 'rewrite', 'picture'])).optional(),
...
export const addSetsToTaskSchema = z.object({
  setIds: z.array(z.string().length(24, "Invalid object ID")).min(1, "At least one set is required")
});
